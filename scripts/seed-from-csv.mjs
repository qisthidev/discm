...
function loadPersonalities() {
  ...
  if (existsSync(csvPath)) {
    ...
    for (const r of required)
      if (!(r in idx))
        throw new Error(`Missing column in personalities.csv: ${r}`);
    return rest.map((row) => ({
      id: toInt(row[idx.id]),
...
function loadPatterns() {
  ...
  if (existsSync(csvPath)) {
    ...
    for (const r of required)
      if (!(r in idx)) throw new Error(`Missing column in patterns.csv: ${r}`);
    return rest.map((row) =>
      Object.fromEntries(
        Object.keys(idx).map((k) => [
          k,
          k === "id" ? toInt(row[idx[k]]) : row[idx[k]],
        ])
      )
    );
  }
...
if (personalities) {
  ...
  const invalid = personalities.filter(
    (p) => !marks.has(p.mostMark) || !marks.has(p.leastMark)
  );
  if (invalid.length) {
    throw new Error(
      "Invalid marks found in personalities seed (allowed: D,I,S,C,#)"
    );
  }
  writeFileSync(
