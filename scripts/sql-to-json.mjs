...
const sqlPath = join(root, '..', 'db', 'disc.sql');
...
writeFileSync(join(outDir, 'resultMap.json'), JSON.stringify(resultMap, null, 2));
...
writeFileSync(join(outDir, 'patternMap.json'), JSON.stringify(patternMap, null, 2));
...
const patterns = patternsTuples.map((t) => {
  const [id, name, emotions, goal, judges_others, influences_others, organization_value, overuses, under_pressure, fear, effectiveness, description] = t.map(unquote);
  return { id, name, emotions, goal, judges_others, influences_others, organization_value, overuses, under_pressure, fear, effectiveness, description };
});
